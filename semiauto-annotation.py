...
if __name__ == "__main__":
    ...
    ## prostate cancer
    name = "ChenYuLong"
    ADC = md.read_image("./"+name+"_adc.mha")## ADC image
    GLAND = md.read_image("./"+name+"_gland.mha")## prostate gland segmentation
    T2W = md.read_image("./"+name+"_t2w.mha")## T2W image
    save_path = name+"_ANNO.png"


    ## the 7th slice, need to be improved
    gland = toshown(GLAND.to_numpy()[7]).astype(np.uint8)
    # gland_pixel = count_gland_pixel(gland)
    # print "gland_pixel",gland_pixel


    adc = toshown(ADC.to_numpy()[7]).astype(np.uint8)
    ...
    t2w = toshown(T2W.to_numpy()[7]).astype(np.uint8)
